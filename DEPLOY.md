# Deploying AnatolianCraft: Kuzeydere on AWS

> **Levy Street production** is deployed via Ansible, not this document:
> the `batidere_game` role in the internal `ansible-scripts` repo runs
> the stack on `idyllic-games-prod` behind nginx + certbot at
> your production domain. Re-running
...
`https://github.com/AnubisZk/anadolucraft-kuzeydere.git` anonymously. If you
...
```bash
ssh ubuntu@<elastic-ip> sudo tail -f /var/log/batidere-setup.log
```
...
A nightly `pg_dump` runs at 03:15 UTC via `/etc/cron.d/batidere-backup`,
writing gzipped dumps to `/var/backups/batidere/` and keeping 14 days.

Restore (stack must be up):

```bash
gunzip -c /var/backups/batidere/batidere-2026-06-10.sql.gz \
  | sudo docker exec -i batidere-db psql -U batidere batidere
```

For off-box safety, sync the directory to S3 occasionally:
`aws s3 sync /var/backups/batidere s3://your-bucket/batidere/`.

## Operational notes

- **Secrets**: the Postgres password is generated at first boot into
  `/opt/batidere/.env` (mode 600, gitignored). Nothing else to manage.
...
- Logs: `sudo docker compose -f /opt/batidere/docker-compose.yml logs -f game`.
...
- **Production**: point your admin hostname at the instance
...
sudo docker exec batidere-db psql -U batidere batidere \
