#!/bin/bash
# AnatolianCraft: Kuzeydere — EC2 first-boot setup (cloud-init user data).
#
...
# Admin dashboard domain (e.g. "admin.example.com"), also with an
...
# ---------------------------------------------------------------------------
REPO="https://github.com/AnubisZk/anadolucraft-kuzeydere.git"
APP_DIR="/opt/batidere"
BACKUP_DIR="/var/backups/batidere"

set -euo pipefail
exec > >(tee -a /var/log/batidere-setup.log) 2>&1
echo "=== AnatolianCraft: Kuzeydere setup started: $(date -u) ==="
...
cat > /usr/local/bin/batidere-backup <<'BACKUP'
#!/bin/bash
set -euo pipefail
BACKUP_DIR="/var/backups/batidere"
mkdir -p "$BACKUP_DIR"
docker exec batidere-db pg_dump -U batidere batidere \
  | gzip > "$BACKUP_DIR/batidere-$(date +%F).sql.gz"
find "$BACKUP_DIR" -name '*.sql.gz' -mtime +14 -delete
BACKUP
chmod +x /usr/local/bin/batidere-backup
echo "15 3 * * * root /usr/local/bin/batidere-backup" > /etc/cron.d/batidere-backup

echo "=== AnatolianCraft: Kuzeydere setup finished: $(date -u) ==="
