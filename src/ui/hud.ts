...
export class Hud {
  ...
  private itemTooltip(item: ItemDef): string {
    ...
    if (item.slot) {
      const slotNames: Record<string, string> = { mainhand: 'Ana El', chest: 'Göğüs', legs: 'Bacak', feet: 'Ayak' };
      html += `<div class="tt-sub">${slotNames[item.slot]}</div>`;
    }
    if (item.weapon) {
      const dps = ((item.weapon.min + item.weapon.max) / 2 / item.weapon.speed).toFixed(1);
      html += `<div class="tt-stat">${item.weapon.min} - ${item.weapon.max} Hasar&nbsp;&nbsp;Hız ${item.weapon.speed.toFixed(1)}</div>`;
      html += `<div class="tt-stat">(saniyede ${dps} hasar)</div>`;
      if (item.weapon.dagger) html += `<div class="tt-sub">Hançer</div>`;
    }
    if (item.stats) {
      for (const [k, v] of Object.entries(item.stats)) {
        if (k === 'armor') html += `<div class="tt-stat">${v} Zırh</div>`;
        else { const tr: Record<string,string> = { str: 'Kuvvet', agi: 'Çeviklik', sta: 'Dayanıklılık', int: 'Zekâ', spi: 'Ruh' }; html += `<div class="tt-green">+${v} ${tr[k] ?? k}</div>`; }
      }
    }
    if (item.foodHp) html += `<div class="tt-desc">Kullan: 18 saniyede ${item.foodHp} can yeniler. Yemek yerken oturmaya devam etmelisin.</div>`;
    if (item.drinkMana) html += `<div class="tt-desc">Kullan: 18 saniyede ${item.drinkMana} mana yeniler. İçerken oturmaya devam etmelisin.</div>`;
    if (item.kind === 'quest') html += `<div class="tt-desc">Görev Eşyası</div>`;
    if (item.requiredClass) html += `<div class="tt-sub">Sınıflar: ${item.requiredClass.map((c) => CLASSES[c].name).join(', ')}</div>`;
    if (item.sellValue > 0) html += `<div class="tt-sub">Satış fiyatı: ${formatMoney(item.sellValue)}</div>`;
    return html;
  }

  private abilityTooltip(res: ResolvedAbility): string {
    const a = res.def;
    const resName = this.sim.player.resourceType === 'rage' ? 'Öfke' : this.sim.player.resourceType === 'energy' ? 'Enerji' : 'Mana';
    let dmgText = '';
    ...
    html += `<div class="tt-sub">Kademe ${res.rank}</div>`;
    const costLine: string[] = [];
    if (res.cost > 0) costLine.push(`${res.cost} ${resName}`);
    if (a.range > 0) costLine.push(`${a.minRange ? a.minRange + '-' : ''}${a.range} m menzil`);
    if (costLine.length) html += `<div class="tt-stat">${costLine.join(' &nbsp; ')}</div>`;
    const castLine: string[] = [];
    castLine.push(a.channel ? `Kanalize (${a.channel.duration} sn)` : res.castTime > 0 ? `${res.castTime} sn okuma` : 'Anlık');
    if (a.cooldown > 0) castLine.push(`${a.cooldown} sn bekleme`);
  ...
  update(): void {
    ...
    if (target && target.kind !== 'object') {
      ...
      $('#tf-elite-tag').textContent = MOBS[target.templateId]?.boss ? 'BOSS' : 'ELİT';
      ...
      $('#tf-hp-text').textContent = target.dead ? 'Ölü' : `${target.hp} / ${target.maxHp}`;
    ...
    if (p.castingAbility) {
      ...
      (cb.querySelector('.label') as HTMLElement).textContent =
        p.eating && p.drinking ? 'Yiyor ve İçiyor…' : p.eating ? 'Yiyor…' : 'İçiyor…';
    } else {
    ...
    $('#xpbar .label').textContent = p.level >= MAX_LEVEL ? 'AZAMİ SEVİYE' : `${sim.xp} / ${xpNeed} TP (%${Math.floor(xpFrac * 100)})`;
      ...
      switch (ev.type) {
        case 'damage': {
          ...
          if (ev.kind === 'miss' || ev.kind === 'dodge') {
            this.fct(tgt, ev.kind === 'miss' ? 'Iska' : 'Savuşturma', isPlayerTarget ? '#bbb' : '#fff', false);
            if (isPlayerSource) {
              this.log(`${ev.ability ?? 'Saldırın'}: ${tgt.name} ${ev.kind === 'miss' ? 'ıskalandı' : 'savuşturdu'}.`, '#ccc');
              audio.meleeMiss();
            }
            break;
          }
          if (isPlayerSource && !isPlayerTarget) {
            ...
            this.log(`${ev.ability ?? 'Saldırın'} ${tgt.name} hedefine ${ev.amount} hasar verdi${ev.crit ? ' (Kritik)' : ''}.`, ev.ability ? '#ffe97a' : '#eee');
            ...
            this.log(`${src?.name ?? 'Bir şey'} sana ${ev.amount} hasar verdi${ev.crit ? ' (Kritik)' : ''}.`, '#ff8877');
            audio.hitTaken();
          }
          break;
        }
        ...
        case 'death': {
          ...
          if (e && ev.entityId !== sim.playerId) this.log(`${e.name} ölüyor.`, '#aaa');
          break;
        }
        case 'xp': {
          this.fct(sim.player, `+${ev.amount} XP`, '#b974ff', false);
          this.log(`${ev.amount} tecrübe puanı kazandın.`, '#a980d8');
          break;
        }
        case 'levelup': {
        ...
        case 'loot': {
          ...
          if (ev.text.includes('yağmaladın') || ev.text.includes('satıldı')) audio.coin();
          else audio.lootItem();
        ...
        case 'questDone':
          ...
          break;
        case 'chat':
          if (ev.channel === 'party') this.log(`[Grup] ${ev.from}: ${ev.text}`, '#7fd4ff');
        ...
        case 'heal2': {
          ...
          if (tgt && ev.amount > 0) {
            ...
            if (ev.sourceId === sim.playerId) {
              this.log(`${ev.ability} ${ev.targetId === sim.playerId ? 'seni' : tgt.name + ' hedefini'} ${ev.amount} iyileştirdi${ev.crit ? ' (Kritik)' : ''}.`, '#7fdc4f');
            }
          }
          break;
        }
        case 'partyInvite':
          audio.questAccept();
          this.showPrompt(`<b>${ev.fromName}</b> seni grubuna davet ediyor.`, 'Gruba Katıl',
            () => this.sim.partyAccept(), () => this.sim.partyDecline());
          break;
        case 'tradeRequest':
          audio.click();
          this.showPrompt(`<b>${ev.fromName}</b> seninle takas yapmak istiyor.`, 'Takası Aç',
            () => this.sim.tradeAccept(), () => { /* let it expire */ });
          break;
        case 'duelRequest':
          audio.aggro();
          this.showPrompt(`<b>${ev.fromName}</b> seni düelloya davet etti!`, 'Düelloyu Kabul Et',
        ...
        case 'duelEnd':
          ...
          this.log(`${ev.winnerName}, düelloda ${ev.loserName} oyuncusunu yendi.`, '#fa6');
          audio.levelUp();
          break;
        case 'log': this.log(ev.text, ev.color ?? '#ccc'); break;
        case 'playerDeath': {
          this.log('Öldün.', '#ff4444');
          audio.death();
          break;
        }
        case 'respawn': this.log('Kendini yeniden dinlenmiş ve sapasağlam hissediyorsun.', '#7fdc4f'); break;
        case 'castStart': {
        ...
        case 'aura': {
          const tgt = sim.entities.get(ev.targetId);
          if ((ev.name === 'Polymorph' || ev.name === 'Şekil Değiştirme') && ev.gained) audio.sheep();
          if (ev.targetId === sim.playerId) {
            this.log(ev.gained ? `${ev.name} kazandın.` : `${ev.name} üzerinden kalkıyor.`, '#d8a0d8');
          } else if (tgt && ev.gained) {
            this.log(`${tgt.name}, ${ev.name} etkisine girdi.`, '#d8a0d8');
          }
          break;
        }
      }
  ...
  private renderGossip(npc: Entity): void {
    ...
    html += `<div class="qd-text">"${(def?.greeting ?? 'Selamlar.').replace('$C', CLASSES[this.sim.cfg.playerClass].name.toLowerCase())}"</div>`;
    ...
    if (npc.vendorItems.length > 0) {
      html += `<div class="qd-list-item" data-vendor="1"><span style="color:#9fdc7f">$</span> Mallarına bir göz atayım.</div>`;
    }
    el.innerHTML = html;
  ...
  private renderQuestDetail(npc: Entity, questId: string): void {
    ...
    let html = `<div class="panel-title"><span>${quest.name}${quest.suggestedPlayers ? ` <span style="color:#f96;font-size:11px">(Önerilen oyuncu: ${quest.suggestedPlayers})</span>` : ''}</span><span class="x-btn" data-close>✕</span></div>`;
    html += `<div class="qd-text">${text}</div>`;
    if (state !== 'ready') {
      const qp = this.sim.questLog.get(questId);
      html += `<div class="qd-sub">Hedefler</div>`;
      html += quest.objectives.map((o, i) => `<div class="qd-obj">&bull; ${o.label}: ${qp ? Math.min(qp.counts[i], o.count) : 0}/${o.count}</div>`).join('');
    }
    html += `<div class="qd-sub">Ödüller</div>`;
    html += `<div class="qd-obj">${quest.xpReward} tecrübe &nbsp; ${this.moneyHtml(quest.copperReward)}</div>`;
    ...
    if (state === 'available') {
      ...
      const btn = document.createElement('button');
      btn.className = 'btn';
      btn.textContent = 'Görevi Tamamla';
  ...
  private renderVendor(): void {
    ...
    hint.textContent = 'Bu pencere açıkken satmak için çantandaki bir eşyaya tıkla.';
    el.appendChild(hint);
  ...
  renderBags(): void {
    const el = $('#bags');
    const sim = this.sim;
    el.innerHTML = `<div class="panel-title"><span>Çantalar</span><span class="x-btn" data-close>✕</span></div>`;
    const grid = document.createElement('div');
    grid.className = 'bag-grid';
    if (sim.inventory.length === 0) {
      grid.innerHTML = `<div style="font-size:12px;color:#887c5c;padding:6px">Çantaların boş.</div>`;
    }
    for (const s of [...sim.inventory]) {
      ...
      this.attachTooltip(row, () => {
        let extra = '';
        if (this.tradeOpen) extra = '<div class="tt-sub">Takasta sunmak için tıkla</div>';
        else if (this.vendorOpen) extra = '<div class="tt-sub">Satmak için tıkla</div>';
        else if (item.kind === 'weapon' || item.kind === 'armor') extra = '<div class="tt-sub">Kuşanmak için tıkla</div>';
        else if (item.kind === 'food' || item.kind === 'drink') extra = '<div class="tt-sub">Tüketmek için tıkla</div>';
  ...
  renderChar(): void {
    ...
    let html = `<div class="panel-title"><span>${p.name} <span style="color:#998d6a;font-size:11px">Seviye ${p.level} ${cls.name}</span></span><span class="x-btn" data-close>✕</span></div>`;
    ...
    html += `<div class="char-stats">
      <span>Kuvvet: <b>${p.stats.str}</b></span><span>Zırh: <b>${p.stats.armor}</b></span>
      <span>Çeviklik: <b>${p.stats.agi}</b></span><span>Saldırı Gücü: <b>${p.attackPower}</b></span>
      <span>Dayanıklılık: <b>${p.stats.sta}</b></span><span>Hasar/sn: <b>${dps.toFixed(1)}</b></span>
      <span>Zekâ: <b>${p.stats.int}</b></span><span>Kritik Şansı: <b>%${(p.critChance * 100).toFixed(1)}</b></span>
      <span>Ruh: <b>${p.stats.spi}</b></span><span>Savuşturma: <b>%${(p.dodgeChance * 100).toFixed(1)}</b></span>
    </div>`;
    el.innerHTML = html;
    const col = el.querySelector('#equip-col')!;
    const slots: { key: 'mainhand' | 'chest' | 'legs' | 'feet'; name: string }[] = [
      { key: 'mainhand', name: 'Ana El' },
      { key: 'chest', name: 'Göğüs' },
      { key: 'legs', name: 'Bacak' },
      { key: 'feet', name: 'Ayak' },
    ];
    for (const slot of slots) {
      ...
      row.innerHTML = `${item ? this.itemIcon(item) : `<img class="item-icon" style="border-color:#444" src="${iconDataUrl('item', 'slot_empty')}" alt="" draggable="false">`}
        <div><div class="slot-name">${slot.name}</div><div class="slot-item" style="color:${qColor}">${item ? item.name : 'Boş'}</div></div>`;
  ...
  renderSpellbook(): void {
    const el = $('#spellbook');
    const sim = this.sim;
    el.innerHTML = `<div class="panel-title"><span>Büyü Kitabı</span><span class="x-btn" data-close>✕</span></div>`;
    const cls = CLASSES[sim.cfg.playerClass];
    for (const abilityId of cls.abilities) {
      ...
      row.innerHTML = `<div class="spell-icon" style="background-image:url(${iconDataUrl('ability', abilityId)});${locked ? 'filter:grayscale(1) brightness(0.5)' : ''}"></div>
        <div><div class="spell-name" style="${locked ? 'color:#777' : ''}">${def.name}${known && known.rank > 1 ? ` <span style="color:#998d6a;font-size:11px">Kademe ${known.rank}</span>` : ''}</div>
        <div class="spell-sub">${locked ? `Seviye ${def.learnLevel} ile öğrenilir` : describeCost(known!, sim)}</div></div>`;
      if (known) this.attachTooltip(row, () => this.abilityTooltip(known));
      else this.attachTooltip(row, () => `<div class="tt-title" style="color:#999">${def.name}</div><div class="tt-sub">Bunu seviye ${def.learnLevel} olduğunda öğreneceksin.</div>`);
      el.appendChild(row);
    }
    el.querySelector('[data-close]')?.addEventListener('click', () => { el.style.display = 'none'; this.hideTooltip(); });
  }
  ...
  renderQuestLog(): void {
    const el = $('#quest-log-window');
    const sim = this.sim;
    el.innerHTML = `<div class="panel-title"><span>Görev Günlüğü <span style="color:#998d6a;font-size:11px">${sim.questLog.size} etkin &middot; ${sim.questsDone.size} tamamlandı</span></span><span class="x-btn" data-close>✕</span></div>`;
    ...
    if (quests.length === 0) {
      list.innerHTML = '<div style="color:#887c5c;font-size:12px;padding:4px">Etkin görev yok.</div>';
      detail.innerHTML = '<div class="qd-text">İş bulmak için <span class="gold">!</span> işaretli kasabalıları ara.</div>';
    }
    ...
    if (this.selectedQuestLogId) {
      ...
      let html = `<div class="qd-sub" style="font-size:15px">${quest.name}${quest.suggestedPlayers ? ` <span style="color:#f96;font-size:11px">(Önerilen oyuncu: ${quest.suggestedPlayers})</span>` : ''}</div>`;
      ...
      html += `<div class="qd-sub">Rewards</div><div class="qd-obj">${quest.xpReward} tecrübe &nbsp; ${this.moneyHtml(quest.copperReward)}</div>`;
      ...
      abandon.textContent = 'Görevi Bırak';
  ...
  private updatePartyFrames(): void {
    ...
    leave.textContent = 'Gruptan Ayrıl';
  ...
  private updateTradeWindow(): void {
    ...
    acceptBtn.textContent = info.myAccepted ? 'Bekleniyor…' : 'Takası Kabul Et';
...
function describeCost(known: ResolvedAbility, sim: IWorld): string {
  const resName = sim.player.resourceType === 'rage' ? 'Öfke' : sim.player.resourceType === 'energy' ? 'Enerji' : 'Mana';
  const parts: string[] = [];
  if (known.cost > 0) parts.push(`${known.cost} ${resName}`);
  parts.push(known.def.channel ? 'Kanalize' : known.castTime > 0 ? `${known.castTime} sn okuma` : 'Anlık');
