...
const WORLD_SEED = 26061; // fixed: Kuzeydere is a persistent place
...
const MIN_LOADING_VISIBLE_MS = 2200;
...
function setLoadingProgress(done: number, total: number): void {
  ...
  setLoadingStatus(`Dünya yükleniyor… ${done}/${total}`);
}

function hideLoadingScreen(): void {
  const el = $('#loading-screen');
  if (!el.classList.contains('visible')) return;
  const wait = Math.max(0, MIN_LOADING_VISIBLE_MS - (performance.now() - loadingShownAt));
  if (wait > 0) {
    loadingHideTimer = window.setTimeout(() => hideLoadingScreen(), wait);
    return;
  }
...
async function startGame(world: IWorld, offlineSim: Sim | null, online: ClientWorld | null): Promise<void> {
  ...
  showLoadingScreen('Dünya yükleniyor…');
  $('#start-screen').style.display = 'none';
  try {
    await assetsReady((done, total) => setLoadingProgress(done, total));
  } catch (err) {
    fatalOverlay(`Varlıklar yüklenemedi — sayfayı yenilemeyi dene. ${err instanceof Error ? err.message : err}`);
    return;
  }
  setLoadingStatus('Dünyaya giriliyor…');
  ...
  try {
    ...
    fatalOverlay(`Görüntüleyici başlatılamadı — sayfayı yenilemeyi dene. ${err instanceof Error ? err.message : err}`);
    return;
  }
  ...
  function interactKey(): void {
    ...
    hud.showError('Etkileşime girilecek bir şey yok.');
  }
...
function sanitizeOfflineName(raw: string): string {
  const stripped = raw.replace(/[^A-Za-zÇĞİÖŞÜçğıöşü' -]/g, '').replace(/^[^A-Za-zÇĞİÖŞÜçğıöşü]+/, '').slice(0, 16);
  return /^[A-Za-zÇĞİÖŞÜçğıöşü][A-Za-zÇĞİÖŞÜçğıöşü' -]{1,15}$/.test(stripped) ? stripped : 'Zafer';
}
...
function enterWorld(c: CharacterSummary): void {
  ...
  audio.init();
  music.init();
  showLoadingScreen('Diyara bağlanılıyor…');
  ...
  const poll = setInterval(() => {
    if (world.connected && world.entities.has(world.playerId)) {
      ...
      clearInterval(poll);
      world.close();
      fatalOverlay('Dünyaya girilemedi (zaman aşımı). Oyun sunucusu çalışıyor mu?');
    }
  }, 50);
...
function wireStartScreens(): void {
  ...
  creditsModal.addEventListener('click', (ev) => {
    if (ev.target === creditsModal) closeCredits();
  });
  document.addEventListener('keydown', (ev) => {
    if (ev.key === 'Escape' && creditsModal.classList.contains('visible')) closeCredits();
  });
  ...
  $('#btn-create-char').addEventListener('click', async () => {
    ...
    loginError('');
    if (!clsEl) { $('#charselect-error').textContent = 'Bir sınıf seç.'; return; }
    try {
